#!/usr/bin/env python3
"""M40 (PLAN.md 55): the vertex cache's A/B -- tools/m40_chain.sh's A:GAME:ARM:K
...
dec, vertices, the cache's share).
    ...
    m40_ab.py DIR        (DIR holds A-*.log and A-*.csv, plain or .gz)
"""
...
def main():
    ...
    for p in sorted(glob.glob(os.path.join(d, 'A-*.log*'))):
        m = re.match(r'A-(\w+)-(\w+)-(\d+)\.log', os.path.basename(p))
        if not m:
            continue
        g, arm, k = m.groups()
