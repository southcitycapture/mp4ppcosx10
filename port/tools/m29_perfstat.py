#!/usr/bin/env python3
"""medians of a --perfdump CSV per window, drawn and consumed apart (the §42.4 / §44 table's
...
for path in sys.argv[1:]:
    ...
    for name, (a, b) in WIN.items():
        ...
        out.append("%s: consumed %.2f (game %.2f aud %.2f) drawn %.1f (game %.1f gx %.1f rt %.1f dec %.1f) presented %.2f [%d/%d]" % (
            ...
            med([float(r['rt_ms']) for r in d]), med([float(r.get('dec_ms', 0) or 0) for r in d]), pf, len(d), len(w)))
    print("\n  ".join(out))
