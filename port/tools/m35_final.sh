#!/bin/sh
# M35 final: on the final build (~/MarioParty4.app): the md5 run, the whole gallery, then the
# realtime soak the milestone leaves running.  ON THE G4 as the console runner's job.
cd $HOME
D=$HOME/m35/final; mkdir -p $D
...
sleep 2; killall -9 isle 2>/dev/null; sleep 2
export GALLERY_APP=$HOME/MarioParty4.app GALLERY_DIR=$HOME/gallery-m35f
