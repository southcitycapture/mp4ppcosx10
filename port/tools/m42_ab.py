#!/usr/bin/env python3
"""M42 (PLAN.md 57): the levers' A/B -- tools/m42_chain.sh's A:GAME:ARM:K
...
dec, vertices, the cache's share).
    ...
    m42_ab.py DIR        (DIR holds A-*.log and A-*.csv, plain or .gz)
"""
import sys, os, re, glob, csv, gzip, statistics as st
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import fps_board

SCREEN = {'cs': 'mentdll', 'b1': 'w01dll', 'b2': 'w02dll', 'b3': 'w03dll', 'b4': 'w04dll', 'b5': 'w05dll', 'b6': 'w06dll'}
...
def main():
    ...
    print('| scene | arm | presented fps, each run | median | mean | game work / gdec | consumed | rt | dec | vertices | vc |')
    print('|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|')
    for g in sorted(set(k[0] for k in runs)):
        for a in arms:
            ...
            print('| %s | %s | %s | **%.1f** | %.1f | %s / %s | %s | %s | %s | %s | %s |' % (
                SCREEN.get(g, g), a, ' / '.join('%.1f' % x[1] for x in r), st.median(x[1] for x in r),
                st.mean(x[1] for x in r),
