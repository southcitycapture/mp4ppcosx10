#!/usr/bin/env python3
"""M39 (PLAN.md 54), from M35's compare_m35.py (the m39 tag first): regenerate port/docs/gallery/compare.html
...
TAGS = ['m39', 'm35', 'm34', 'm31', 'm30']   # newest first; 'fix' (the M26 fix build) is the floor
tagv = {}
for t in TAGS:
    tagv[t] = {}
...
<title>Mario Party 4 PowerPC port: the port beside the console, every minigame (M26b, M30, M31, M34, M35, M39)</title>
...
<h1>The port beside the console: every minigame, seven positions (M26b, the oracle sweep; M30, M31, M34, M35 and M39 rows regenerated)</h1>
<p>Each row: the port's frame (top: the M26 fix build on the G4, or &mdash; on the rows marked <span class="m30">M30</span> / <span class="m30">M31</span> / <span class="m30">M34</span> / <span class="m30">M35</span> / <span class="m30">M39</span> &mdash; that milestone's build, <code>gallery_chain.sh</code>) over the console's (bottom, Dolphin on littlejelly, the same schedule: board-start walk, four COMs, <code>mg_next</code> and <code>mgNext</code> poked, groups by type) at the same <em>event</em>: the instruction card at instDll +64/+164/+264, the module at entry +60/+400/+1200/+2300, and the results screen 40 frames after the module is left. The console frame's number is Dolphin's dump index. Under each pair: <b>sim</b> = 100·(1 − mean |Δ| / 255) over RGB at 320×240, <b>&gt;8</b> = % of pixels whose worst channel differs by more than 8 levels (ppmdiff's count). The two runs do not share their RNG (the board seeds from the clock at board setup, which the two walks reach at different frames), so play frames differ by the play itself; the card and the intro frames are the frame-exact pairs. PLAN.md §41b has the calibration and the verdicts' reasoning; §45 the M30 rows, §46 the M31 rows, §49 the M34 rows, §50 the M35 rows, §54 the M39 rows (the release candidate, --nomovies) (each regenerated row keeps the earlier verdicts and notes below its own as the before picture).</p>''')
counts = {}; tsv = open(os.path.join(G, 'similarity-m39.tsv'), 'w'); tsv.write('game\t' + '\t'.join(KEYS) + '\n')
