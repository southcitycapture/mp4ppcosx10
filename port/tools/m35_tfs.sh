#!/bin/sh
# M35 (PLAN.md 50.13, 50.14): the indirect warp on GL_ATI_text_fragment_shader
# and the Thwomps' lighting, ON THE G4 as the console runner's job (install as
...
cd "$HOME"
A="${TFS_APP:-$HOME/MarioParty4-tfs.app}/Contents/MacOS/isle"
D="${TFS_DIR:-$HOME/m35/tfs}"; mkdir -p "$D"
IDX="$D/index.txt"
RUNS=${*:-"m405 m417 m434 m425 m425old m448all m432dl m403dl"}
...
for r in $RUNS; do
    case $r in
        ...
        dbg*) run "$r" m405 --tfsdbg "${r#dbg}" --mgdump 400 --mgend 420 ;;
        all*) run "$r" m405 --tfsall 1 --tfsdbg "${r#all}" --mgdump 400 --mgend 420 ;;
        two*) run "$r" m405 --tfsall 2 --tfsdbg "${r#two}" --mgdump 400 --mgend 420 ;;
        p*) run "$r" m434 --tfsdbg "${r#p}" --mgdump 400 --mgend 420 ;;
        w*) run "$r" m417 --tfsdbg "${r#w}" --mgdump 400 --mgend 420 ;;
