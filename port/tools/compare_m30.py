#!/usr/bin/env python3
"""M30 (PLAN.md 45): regenerate port/docs/gallery/compare.html with the M30 build's
frames for the games a cause touched.  The console frames stay; a game with
...
is which event) are read from ~/mp4-sweep-work/picks/.
    ...
    compare_m30.py [--gallery-dir DIR]     (default: port/docs/gallery next to this file)
"""
...
m30 = {}
vp = os.path.join(G, 'verdicts-m30.tsv')
if os.path.exists(vp):
    for line in open(vp):
        p = line.rstrip('\n').split('\t')
        if len(p) >= 3 and p[0].startswith('m4'): m30[p[0]] = (p[1], p[2])
...
<title>Mario Party 4 PowerPC port: the port beside the console, every minigame (M26b, M30)</title>
...
for g in sorted(types):
    ...
    tag = 'm30' if glob.glob(f'{G}/{g}-m30-f*.jpg') else 'fix'
    if tag == 'm30' and g in m30: v, note = m30[g]
    else: v, note = v26, note26
    ...
    H.append(f'<div class="g" id="{g}"><div class="h">{g} &mdash; {names.get(g, g)} <span class="m">({TYPE.get(types[g], types[g])}, type {types[g]})</span> '
             f'<span class="m">§41: {old.get(g, "?")} &rarr; M26b: {v26} &rarr;</span> <span class="{v}">{v}</span>' + (' <span class="m30">M30 build</span>' if tag == 'm30' else '') + '</div>')
    if P:
    ...
    H.append('</div>')
    extra = f'<br><b>M26b ({v26})</b>: {note26}' if tag == 'm30' else ''
