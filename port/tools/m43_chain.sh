#!/bin/sh
# M43 (PLAN.md 58): the last eighteen -- M43's chain (below) plus the
# compiler's arms (a whole bundle each), the PGO training runs and the
# vertex cache's per-array counts.  ON THE G4
# as the console runner's job (M40's chain shape: `cp tools/m43_chain.sh` over
# ~/MarioParty4-chain.app/Contents/MacOS/isle, `g4 use MarioParty4-chain.app;
# g4 run RUNS...`).  Waits M43_SETTLE (90) s first.  One binary
# (~/MarioParty4.app, or M43_APP); ~/m43/NAME.log, NAME.csv, NAME.sample.txt,
# ~/m43/index.txt.
#
#   S:GAME           GAME at real time (the scoreboard's teleport), `sample`d for
#                    M43_SECS (12) s from entry + M43_WAIT (8) s (cs: frame 3000)
...
# compiler's variants, each a whole build); base (the defaults), old (the M43_APP_OLD binary, default the 0.9.10
# bundle ~/MarioParty4-m41.app, with its defaults) or a flag list with ','
# for ' ' (e.g. --nostripes).
cd "$HOME"
[ -f "$HOME/m43.env" ] && . "$HOME/m43.env"
sleep "${M43_SETTLE:-90}"
D="${M43_DIR:-$HOME/m43}"; mkdir -p "$D"
IDX="$D/index.txt"
APP="${M43_APP:-$HOME/MarioParty4.app/Contents/MacOS/isle}"
...
echo "# m43 chain start $(date)  isle md5 $(md5 -q "$APP")" >> "$IDX"
...
sampler() {
    ...
    sample $pid "${M43_SECS:-12}" -file "$D/$name.sample.txt" >/dev/null 2>&1
    grep '^port> status f' "$log" | tail -1 >> "$D/$name.sample-from.txt"
}
...
for r in ${M43_RUNS:-N}; do
    case $r in
        ...
        K:*)
            ...
            g=$(echo $r | cut -d: -f2); k=$(echo $r | cut -d: -f3)
            case $g in cs) w=${M43_PMCWIN_CS:-3000,4800} ;; b*) w=${M43_PMCWIN_B:-8400,10400} ;; *) w=${M43_PMCWIN:-300,1500} ;; esac
            run "K-$g-$k-@pmc" 600 - $BASE --realtime --pmc $k --pmcwin $w $(scene $g) ;;
        FB:*)
...
echo "DONE" >> "$IDX"
[ -n "$M43_LEAVE" ] && exec "$APP" $M43_LEAVE
exit 0
