...
/* M41 (PLAN.md 56): the packed lights.  The per-light block below is ~21
 * instructions (a spot light with its distance attenuation), so six lights
 * come to 159-165 and the card's limit is 128: Archaeologuess's (m447) lamps
 * went to the CPU path, 57% of its drawn frame.  Packed, four lights share
 * one register a lane each -- the squared distances, the diffuse dots, the
 * cone dots, the attenuations -- so a vector instruction serves four lights
 * and only the square roots, the reciprocals and the colour sums stay one a
 * light: ~11 a light.  The per-light constants come transposed, as local
 * parameters (k0, k1, k2, a0, a1, a2 of four lights in a vector each).  The
 * arithmetic is light_channel's, as the scalar block's is; the order of two
 * roundings differs (the dots are taken before the direction is normalised,
 * then scaled by the same 1/d).  Used only for a variant the scalar block
 * does not fit; --nopacklights leaves those to the CPU path, as before. */
static int vp_gen_packed;

static void vp_gen_packed_lights(const VpKey* k, VpBuf* b) {
    static const char lane[] = "xyzw";
    int g, j;
    int spot = k->attn_fn == GX_AF_SPOT && !port_opt.nospot;
    for (g = 0; g * 4 < k->nlights; g++) {
        int n = k->nlights - g * 4 > 4 ? 4 : k->nlights - g * 4;
        char m[6];
        int lb = g * 6; /* the group's six local parameters */
        m[0] = '.';
        memcpy(m + 1, lane, (size_t)n);
        m[n + 1] = 0;
        for (j = 0; j < n; j++) {
            int lp = VPE_LIGHT + VPE_LSTRIDE * (g * 4 + j);
            vpi(b, "SUB t0.xyz, program.env[%d], vp;\n", lp);
            vpi(b, "DP3 d2.%c, t0, t0;\n", lane[j]);
            if (k->diff_fn == GX_DF_CLAMP || k->diff_fn == GX_DF_SIGN) {
                vpi(b, "DP3 nl.%c, nr, t0;\n", lane[j]);
            }
            if (spot) {
                vpi(b, "DP3 sp.%c, t0, program.env[%d];\n", lane[j], lp + 4);
            }
        }
        vpi(b, "MAX d2%s, d2, 1.0e-30;\n", m);
        for (j = 0; j < n; j++) {
            vpi(b, "RSQ t1.%c, d2.%c;\n", lane[j], lane[j]);  /* 1/d            */
        }
        if (k->diff_fn == GX_DF_CLAMP || k->diff_fn == GX_DF_SIGN) {
            vpi(b, "MUL nl%s, nl, t1;\n", m);                  /* N . unit dir   */
            if (k->diff_fn == GX_DF_CLAMP) {
                vpi(b, "MAX nl%s, nl, 0.0;\n", m);
            }
        } else {
            vpi(b, "MOV nl%s, 1.0;\n", m);
        }
        if (k->attn_fn != GX_AF_NONE) {
            vpi(b, "MUL t0%s, d2, t1;\n", m);                                  /* d */
            vpi(b, "MAD t0%s, program.local[%d], t0, program.local[%d];\n", m, lb + 1, lb + 0);
            vpi(b, "MAD t0%s, program.local[%d], d2, t0;\n", m, lb + 2);
            vpi(b, "MAX t0%s, t0, 1.0e-20;\n", m);
            for (j = 0; j < n; j++) {
                vpi(b, "RCP t0.%c, t0.%c;\n", lane[j], lane[j]);
            }
            vpi(b, "MIN t0%s, t0, 1.0;\n", m);
            vpi(b, "MUL nl%s, nl, t0;\n", m);
        }
        if (spot) {
            vpi(b, "MUL sp%s, sp, t1;\n", m);                                   /* ldir . dir */
            vpi(b, "MAX sp%s, -sp, 0.0;\n", m);                                 /* c     */
            vpi(b, "MUL t0%s, sp, sp;\n", m);                                   /* c^2   */
            vpi(b, "MAD t0%s, program.local[%d], t0, program.local[%d];\n", m, lb + 5, lb + 3);
            vpi(b, "MAD t0%s, program.local[%d], sp, t0;\n", m, lb + 4);
            vpi(b, "MAX t0%s, t0, 0.0;\n", m);
            vpi(b, "MUL nl%s, nl, t0;\n", m);
        }
        for (j = 0; j < n; j++) {
            int lp = VPE_LIGHT + VPE_LSTRIDE * (g * 4 + j);
            vpi(b, "MAD ac%s, program.env[%d], nl.%c, ac;\n", k->alit ? "" : ".xyz", lp + 1, lane[j]);
        }
    }
}
...
static void vp_gen(const VpKey* k, VpBuf* b) {
    ...
    if (!k->lit) {
        ...
        if (vp_gen_packed) {
            vp_gen_packed_lights(k, b);
        }
        for (i = 0; i < k->nlights && !vp_gen_packed; i++) {
...
static VpVariant* vp_lookup(const VpKey* k) {
    ...
    memset(&b, 0, sizeof(b));
    vp_gen(k, &b);
    if (b.s && b.instr > vpl.max_native_instr && k->lit && !k->hilite && !k->spec0 &&
        k->nlights >= 2 && k->nlights <= VPE_NLIGHTS && !port_opt.nopacklights) {
        /* M41: the scalar lights do not fit; the packed ones may */
        VpBuf pb;
        memset(&pb, 0, sizeof(pb));
        vp_gen_packed = 1;
        vp_gen(k, &pb);
        vp_gen_packed = 0;
        if (pb.s && pb.instr <= vpl.max_native_instr) {
            if (port_opt.vprogstats || port_opt.vproglog) {
                port_log("port> vprog: variant %d: %d instructions with the lights one at a "
                         "time, %d packed four to a register (M41)\n",
                         vp_nvariants, b.instr, pb.instr);
            }
            free(b.s);
            b = pb;
            v->packed = 1;
            stat_packed_variants++;
        } else {
            free(pb.s);
        }
    }
...
void gx_vprog_bind(const GxXfDesc* d) {
    ...
    if (key.lit) {
        ...
        if (v->packed) {
            /* M41: the packed lights' constants, transposed: local g*6+c is
             * coefficient c (k0 k1 k2 a0 a1 a2) of lights 4g..4g+3, a lane each */
            int g, c, j;
            for (g = 0; g * 4 < nl && g < 2; g++) {
                for (c = 0; c < 6; c++) {
                    float q[4] = {c == 0 ? 1.0f : 0.0f, c == 0 ? 1.0f : 0.0f,
                                  c == 0 ? 1.0f : 0.0f, c == 0 ? 1.0f : 0.0f};
                    int idx = g * 6 + c;
                    for (j = 0; j < 4 && g * 4 + j < nl; j++) {
