...
/* M43 (PLAN.md 58.2): a twin called on the render thread itself -- by the
...
static int rt_skip_draws;
void rt_set_skip_draws(int on) { rt_skip_draws = on; }
int rt_threaded(void) { return rt_recording && mode >= 2 && thread_up; }
...
void rt_glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* p) {
    if (!RT_REC()) { glColorPointer(size, type, stride, p); return; }
...
void rt_glNormalPointer(GLenum type, GLsizei stride, const GLvoid* p) {
    if (!RT_REC()) { glNormalPointer(type, stride, p); return; }
...
void rt_glTexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* p) {
    if (!RT_REC()) { glTexCoordPointer(size, type, stride, p); return; }
...
void rt_glDrawRangeElements(GLenum mode_, GLuint lo, GLuint hi, GLsizei n, GLenum type,
                            const GLvoid* idx) {
    if (!RT_REC()) { glDrawRangeElements(mode_, lo, hi, n, type, idx); return; }
    {
...
void rt_glGenTextures(GLsizei n, GLuint* out) {
    ...
    GLsizei i;
    if (!RT_REC()) { glGenTextures(n, out); return; }
    for (i = 0; i < n; i++) {
        out[i] = next_name++;
        st_names++;
    }
...
void rt_glDeleteTextures(GLsizei n, const GLuint* names) {
    if (!RT_REC()) { glDeleteTextures(n, names); return; }
    {
...
void rt_glTexImage2D(GLenum target, GLint level, GLint ifmt, GLsizei w, GLsizei h, GLint border,
    ...
    if (!RT_REC()) { glTexImage2D(target, level, ifmt, w, h, border, fmt, type, px); return; }
    {
...
void rt_glCopyTexSubImage2D(GLenum target, GLint level, GLint xo, GLint yo, GLint x, GLint y,
                            GLsizei w, GLsizei h) {
    if (!RT_REC()) { glCopyTexSubImage2D(target, level, xo, yo, x, y, w, h); return; }
...
void rt_glReadPixels(GLint x, GLint y, GLsizei w, GLsizei h, GLenum fmt, GLenum type, GLvoid* out) {
    if (!RT_REC()) { glReadPixels(x, y, w, h, fmt, type, out); return; }
...
void rt_glGetTexImage(GLenum target, GLint level, GLenum fmt, GLenum type, GLvoid* out) {
    if (!RT_REC()) { glGetTexImage(target, level, fmt, type, out); return; }
...
void rt_ext_local_param4fv(GLenum target, GLuint idx, const GLfloat* v) {
    if (!RT_REC()) { x_ProgramLocalParameter4fvARB(target, idx, v); return; }
...
void rt_ext_env_params4fv(GLenum target, GLuint idx, GLsizei n, const GLfloat* v) {
    if (!RT_REC()) { x_ProgramEnvParameters4fvEXT(target, idx, n, v); return; }
    {
...
void rt_decode_record(const GxDecJob* j) {
    ...
    st_dec_records++;
    {
        /* M43: the arrays it reads, for the rewriters' precise join */
        int k;
        for (k = 0; k < j->nplan; k++) {
            if (j->plan[k].idx && j->plan[k].base) {
                vj_note(j->plan[k].base, wr);
            }
        }
    }
    done();
...
void rt_vtx_rewrite_range(const void* p, unsigned long n, const char* who) {
    ...
    st_vj_waits++;
    rt_decode_join_pos(need, who);
}
void port_vtx_rewrite_done(void) {
    gx_vc_epoch++; /* M40: the vertex cache re-hashes what is about to be rewritten */
}
...
void rt_stop(void) {
    ...
    gx_rtgx_release(); /* M43: the game thread's shadow again, before the last join */
    if (mode == 1) {
        replay_upto(wr);
    } else if (thread_up) {
        rt_join("shutdown");
